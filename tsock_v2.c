...
/*----------------UDP-----------------------*/
int udp_source (int port, int nb_message, int lg_msg, char* nom_station, char* message)
{
	int sock;
	struct hostent *hp;
...
int udp_puits(int port, int nb_message, int lg_msg, char* nom_station, char* message)
{
	int sock; 
	...
	adr_local.sin_port = port; //num de port
...
void tcp_source(int port, int nb_message, int lg_msg, char* nom_station, char* message)
{
	...
	/*Création d'un socket local*/
    	...
    	//Affectation domaine et numero de port
       	...
       	adr_distant.sin_port = port; //num de port
   		//Affectation adresse IP
	...
	while (connect(sock, (struct sockaddr*) &adr_distant, lg_adr_distant) == -1)
	{
...
//	port = htons(port);
	/*Recuperation du nom de station en pénultième paramètre passé*/
	nom_station = argv[argc-2];
	
	printf("__________%i_________\n",port);

	if ((port == -1) || (port == 0))
	{
		printf("Precisez le port (!= 0) \nusage: cmd [-p|-s][-n ##][-u][station][port]\n");
		exit(1);
	}
	...
	else
	{
		...
		else if (protocole == 0) /*Gestion puits TCP*/
		{
