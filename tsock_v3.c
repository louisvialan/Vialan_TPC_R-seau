...
/*void emetteur(int port, int id, int nb_message, int lg_msg, char* nom_station, char* message)
{
	...
	/*if (nb_message != -1)
	{
		printf("EMETTEUR : lg_mesg-emis=%i, port=%i, nb_envois= %i, "
		"TP=TCP, dest=%s\n", lg_msg, port, nb_message, nom_station);
	}
	else
	{
		printf("EMETTEUR : lg_mesg-emis=%i, port=%i, nb_envois=10 (par défaut), "
...
void recepteur(int port, int id, char* nom_station, char* message)
{
	...
	/* ENVOI DU PDU */
	/*int sock = socket(AF_INET, SOCK_STREAM, 0); //Représentation interne du socket local
	struct sockaddr_in adr_serveur; //Adresse du socket distant
