...
void emetteur(int port, int id, int nb_message, int lg_msg, char* nom_station, char* message)
{
	...
	/*Entete dans le terminal*/
	    if (nb_message != -1)
	    {
		    printf("SOURCE : lg_mesg-emis=%i, port=%i, nb_envois= %i, "
		    "TP=TCP, dest=%s\n", lg_msg, port, nb_message, nom_station);
	    }
	    else
	    {
		    printf("SOURCE : lg_mesg-emis=%i, port=%i, nb_envois=10 (par défaut), "
...
void bal (int port, int lg_msg, char* message)
{
	...
	int lg_adr_local = sizeof(adr_local);
	    ...
	    while(1)
	        {
	            if ((sock_com = accept(sock, (struct sockaddr*) &adr_client, (socklen_t *) &lg_adr_distant)) == -1)
		        {
			        printf("Echec du accept.\n");
			        exit(1);
		        }
